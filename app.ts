...
console.log(chalk.bgBlueBright.italic.white( 
  "  Welcome to the Rock Paper Scissor Game created by REHAN KHAN. I welcome you to this game with the bottom of my heart. This game that you will play, will be between you and the computer. So play it and remember me in your prayers. "
));

while (condition) {
  const userChoice: any = await inquirer.prompt({
    name: "askUser",
    type: "list",
  ...
  let arr: string[] = ["Rock", "Paper", "Scissor"];
  // This function will select the element randomly from the array.
  function computerChoice(arr: any) {
  ...
  if (userChoice.askUser === computer) {
    ...
    (userChoice.askUser === "Paper" && computer === "Rock") ||
  ...
  const moreGame: any = await inquirer.prompt({
    name: "Question",
